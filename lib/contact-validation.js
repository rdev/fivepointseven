...
module.exports = function contactValidation(body) {
	...
	if (
		...
		message.toLowerCase().includes('advertise') ||
		message.toLowerCase().includes('million') ||
		message.toLowerCase().includes('billion') ||
		message.toLowerCase().includes('promoting') ||
	...
	if (message.toLowerCase().includes('upwork')) {
		err.push({
			type: 'message',
